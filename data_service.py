...
Fixes v4:
  - Token pre-flight test: validates token with 1 API call before bulk fetch
  - If token is invalid/401 → clears session token → forces re-login immediately
  - Reduced timeout 20s→8s, retries 3→2 so failures are fast
  - Timezone fix: tz_convert(None) for IST timestamps
  - Instrument master: filter by NSE_EQ| prefix (reliable)
  - st.stop() guard if empty data → prevents IndexError crash in main app
"""
...
def _load_instrument_map() -> dict:
    ...
    try:
        ...
        # Filter: NSE Equity only — instrument_key always starts with "NSE_EQ|"
...
# ─────────────────────────────────────────────────────────────
# SECTION B - TOKEN VALIDATION (pre-flight test)
# ─────────────────────────────────────────────────────────────
def _validate_token(access_token: str) -> bool:
    """
    Make ONE real API call to check if the token is still valid.
    Uses RELIANCE (a reliable stock) as a test symbol.
    Returns True if token works, False if 401/403/any error.
    """
    test_key = "NSE_EQ%7CINE002A01018"   # RELIANCE instrument key, pipe encoded
    url = (
        f"https://api.upstox.com/v3/historical-candle"
        f"/{test_key}/day/2024-01-05/2024-01-01"
    )
    headers = {
    ...
    try:
        ...
        if resp.status_code in (401, 403):
            return False
        # 200 or even 400 (bad request) means token itself is valid
        return True
    except Exception:
        return False
...
# ─────────────────────────────────────────────────────────────
# SECTION C - UPSTOX HISTORICAL DATA FETCHER (LIVE, V3 API)
# ─────────────────────────────────────────────────────────────
def _fetch_upstox_history_live(
    ...
    end_date: datetime,
    interval: str = "day",
    retries: int = 2,       # reduced from 3 → faster failure
):
    ...
    for attempt in range(retries):
        try:
            resp = requests.get(url, headers=headers, timeout=8)   # reduced from 20→8s
            ...
            if resp.status_code in (401, 403):
                # Token became invalid mid-run → raise so caller can handle
            ...
            # TIMEZONE FIX: Upstox returns "+05:30" timestamps → strip tz
...
# ─────────────────────────────────────────────────────────────
# SECTION D - ZERODHA PLACEHOLDER (Phase-1)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# SECTION E - MOCK DATA GENERATOR (Zerodha only)
# ─────────────────────────────────────────────────────────────
...
def fetch_upstox(symbols, start_date, end_date, chunk_size, progress_bar, status_text):
    ...
    if not access_token:
        ...
        st.error("Please complete Upstox login in the sidebar first, then click 'Start Data Download' again.")
        st.stop()

    # ── STEP 2: PRE-FLIGHT TOKEN TEST ───────────────────────────
    # Validate token with 1 real API call BEFORE processing 2000+ symbols
    status_text.text("Validating Upstox token...")
    token_ok = _validate_token(access_token)

    if not token_ok:
        # Token is stale → clear it so sidebar shows login prompt again
        st.session_state.pop("upstox_token_data", None)
        progress_bar.progress(0.0)
        status_text.text("Token expired.")
        st.error(
            "Upstox token has expired or is invalid.\n\n"
            "Please re-login from the sidebar (Step 1 → paste code → Generate Token), "
            "then click 'Start Data Download' again."
        )
        st.stop()

    st.sidebar.success("Token validated OK")

    # ── STEP 3: Load instrument master ──────────────────────────
    instrument_map = _load_instrument_map()
    if not instrument_map:
        st.error("Could not load Upstox instrument master. Check internet connection and try again.")
        st.stop()

    # ── STEP 4: Fetch per symbol ─────────────────────────────────
    symbol_dfs  = {}
    failed      = []
    not_found   = 0
    total       = len(symbols)
    ...
    for i, sym in enumerate(symbols):
        ...
        try:
            ...
            # Token went invalid mid-run
            st.session_state.pop("upstox_token_data", None)
            st.error(
                f"Token expired during download at symbol {sym}.\n"
                "Please re-login from sidebar and retry."
            )
            st.stop()
    ...
    status_text.text(
        f"Upstox done - {fetched}/{total} fetched  |  "
        f"Not in master: {not_found}"
    )

    close, high, volume = _build_wide_frames(symbol_dfs)

    # Guard: no data → stop cleanly (prevents IndexError crash in main app)
    if close.empty:
        st.error(
            "No data fetched from Upstox.\n\n"
            "Possible reasons:\n"
            "1. Token expired — re-login from sidebar\n"
            "2. All symbols missing from instrument master\n"
            "3. Network error\n\n"
