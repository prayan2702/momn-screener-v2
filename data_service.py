...
Fixes v5:
  - Progress updates every 10 symbols (was every 5% = ~105 symbols for AllNSE)
  - Debug expander shows first API response so you can see what Upstox returns
  - Reduced sleep 0.12s → 0.05s (Upstox allows 25 req/sec on live accounts)
  - Timezone fix: tz_convert(None) for IST timestamps
  - Token pre-flight validation before bulk fetch
  - st.stop() guard prevents IndexError crash if no data returned
"""
...
# ─────────────────────────────────────────────────────────────
# SECTION B — TOKEN VALIDATION (pre-flight)
# ─────────────────────────────────────────────────────────────
def _validate_token(access_token: str) -> bool:
    """One real API call to verify the token works."""
    url = (
        "https://api.upstox.com/v3/historical-candle"
        "/NSE_EQ%7CINE002A01018/day/2024-01-05/2024-01-01"
    )
...
# ─────────────────────────────────────────────────────────────
# SECTION C — SINGLE SYMBOL FETCHER (V3 API)
# ─────────────────────────────────────────────────────────────
def _fetch_upstox_history_live(
    ...
    """
    Fetch OHLCV for one symbol from Upstox V3 Historical Candle API.
    URL: GET /v3/historical-candle/{instrument_key}/{interval}/{to_date}/{from_date}
...
# ─────────────────────────────────────────────────────────────
# SECTION D — DEBUG: Test first symbol response
# ─────────────────────────────────────────────────────────────
def _debug_first_symbol(instrument_key, access_token, start_date, end_date):
    """
    Makes a raw API call for the first symbol and shows full response in sidebar.
    This helps diagnose what Upstox is actually returning.
    """
    encoded_key   = instrument_key.replace("|", "%7C")
    from_date_str = start_date.strftime("%Y-%m-%d")
    to_date_str   = end_date.strftime("%Y-%m-%d")
    url = (
        f"https://api.upstox.com/v3/historical-candle"
        f"/{encoded_key}/day/{to_date_str}/{from_date_str}"
    )
    headers = {"Authorization": f"Bearer {access_token}", "Accept": "application/json"}
    try:
        resp = requests.get(url, headers=headers, timeout=10)
...
# ─────────────────────────────────────────────────────────────
# SECTION E — ZERODHA PLACEHOLDER (Phase-1)
# ─────────────────────────────────────────────────────────────
...
def fetch_upstox(symbols, start_date, end_date, chunk_size, progress_bar, status_text):
    ...
    # STEP 4: Debug — test first symbol BEFORE bulk fetch
    ...
    # STEP 5: Bulk fetch — update UI every 10 symbols
    symbol_dfs = {}
    failed     = []
    not_found  = 0
    total      = len(symbols)
    UPDATE_EVERY = 10   # update progress bar every 10 symbols
    ...
    for i, sym in enumerate(symbols):
        ...
        time.sleep(0.05)   # 20 req/sec — safe for live accounts
    ...
    if close.empty:
        st.error(
            ...
            "Check the 'Debug: First symbol API response' in sidebar to see what Upstox returned.\n\n"
            "Common fixes:\n"
            "1. Re-login (token may have expired)\n"
            "2. Check if your Upstox account has API data access enabled"
        )
        st.stop()
